defmodule StatifierExamples.Charts.Timers.Delivery do
  @moduledoc """
  How a fired timer job gets back into a durable execution.

  `StatifierOban.Timer.Delivery` is a behaviour precisely because whether
  an execution is live is the host's question. The package's default answers it
  from `Statifier.Session`'s registry, and this app has no session
  process to look up: its executions live in SQLite between steps and are
  driven by whichever process happens to be holding the page. So the
  answer here is the stored execution's own status, and the feed-back is one
  more drive of `StatifierExamples.Charts.Durable`.

  Nothing about this module knows a page exists. It is handed an execution id
  ...
  enforcing that the delivery seam's job. An execution that finished, was
  ...
  reason saying which half is missing: an execution whose record carries no
  ...
  the execution started - the last arriving as the storage layer's own
  ...
  @doc """
  Feeds `effect`'s event back into the execution named by `scope`, if it is
  still live.
  """
