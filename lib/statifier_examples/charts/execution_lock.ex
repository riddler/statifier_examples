defmodule StatifierExamples.Charts.ExecutionLock do
  ...
  `StatifierPersistence.Executions` defaults its `serialization:` option to
  `{StatifierPersistence.Serialization.AdapterLock, store}`, which asks
  the storage adapter for its optional `lock_execution/3`.
  ...
  also the `config` `with_execution/3` is called with.
  """
  ...
  @doc """
  Runs `fun` under this app's exclusion for `execution_id`.
  ...
  @spec with_execution(GenServer.server(), String.t(), (-> result)) :: {:ok, result}
        when result: var
  def with_execution(config, execution_id, fun)
      when is_binary(execution_id) and is_function(fun, 0) do
    :ok = GenServer.call(config, {:acquire, execution_id}, @acquire_timeout)

    try do
      {:ok, fun.()}
    after
      GenServer.call(config, {:release, execution_id}, @acquire_timeout)
    end
  end
  ...
  @impl GenServer
  def handle_call({:acquire, execution_id}, {pid, _tag} = from, state) do
    case Map.fetch(state.locks, execution_id) do
      :error -> {:reply, :ok, grant(state, execution_id, pid)}
      {:ok, held} -> {:noreply, enqueue(state, execution_id, held, from)}
    end
  end

  def handle_call({:release, execution_id}, _from, state) do
    {:reply, :ok, hand_over(state, execution_id)}
  end

  @impl GenServer
  def handle_info({:DOWN, owner, :process, _pid, _reason}, state) do
    case Map.fetch(state.owners, owner) do
      {:ok, execution_id} -> {:noreply, hand_over(state, execution_id)}
      :error -> {:noreply, state}
    end
  end
  ...
  defp grant(state, execution_id, pid) do
    owner = Process.monitor(pid)

    %{
      state
      | locks: Map.put(state.locks, execution_id, %{owner: owner, waiting: :queue.new()}),
        owners: Map.put(state.owners, owner, execution_id)
    }
  end

  @spec enqueue(state(), String.t(), held(), waiter()) :: state()
  defp enqueue(state, execution_id, held, from) do
    held = %{held | waiting: :queue.in(from, held.waiting)}

    %{state | locks: Map.put(state.locks, execution_id, held)}
  end
  ...
  defp hand_over(state, execution_id) do
    case Map.fetch(state.locks, execution_id) do
      :error ->
        state
      ...
      {:ok, held} ->
        ...
        case :queue.out(held.waiting) do
          {:empty, _queue} ->
            %{state | locks: Map.delete(state.locks, execution_id)}

          {{:value, {pid, _tag} = from}, waiting} ->
            state = grant(state, execution_id, pid)
            GenServer.reply(from, :ok)
            put_in(state.locks[execution_id].waiting, waiting)
        end
    end
  end
