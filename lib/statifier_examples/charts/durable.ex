defmodule StatifierExamples.Charts.Durable do
  ...
  `datamodel` is the fixture's datamodel document (`sd-ADR-0001`), or
  `nil` for a chart that has none. It is the one option here that does
  **not** change the generated bytes: the compiler reads it to decide the
  data-flow check ADR-0011 defines - which type a path holds at a
  position, and whether what a block reads there is satisfied - and
  nothing it says reaches the SCXML. So chart identity is unaffected by
  it, and a caller that omits one for a chart whose palette declares
  read signatures gets a refusal rather than a differently-hashed chart:
  with no declarations in hand two named types are two opaque strings,
  identity is all the check has left, and the read is refused. Loud is
  the right failure here; the alternative is a chart that compiles to
  bytes nobody checked.
  """
  @spec compile(Document.t(), [Fixture.declaration()], map() | nil) ::
          {:ok, Compiled.t()} | {:error, [Compiler.Finding.t()]}
  def compile(%Document{} = document, declare, datamodel \\ nil) when is_list(declare) do
  ...
  defp root_chart(key) do
    with {:ok, fixture} <- Charts.fixture(key),
         {:ok, %Compiled{} = compiled} <-
           compile(fixture.document, fixture.declare, fixture.datamodel) do
  ...
  # its declarations and `terminate: true` (`compile/3`), a child with
