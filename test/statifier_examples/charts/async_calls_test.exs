defmodule StatifierExamples.Charts.AsyncCallsTest do
  ...
  that has never seen the execution, and cancelled by the clock the chart armed
  around it.
  ...
  # what drives the execution as far as the asynchronous call.
  ...
  # Drives a fresh execution as far as the asynchronous call and leaves it there.
  ...
  # the shipped fixture, then load. Nothing of the execution that wrote it is in
  ...
  # A bare invoke effect, for the two functions that take one without an execution
  ...
  # it: an execution at rest in the MIDDLE of a call. Three facts, and the execution
  ...
  # `:pending` but no job was ever stored; the execution rested on a call nobody
  ...
  # success, the execution stayed `:active` and this went red on the status.
  # Reverted from a backup copy.
  ...
  # execution's own serialization strategy rather than before the call.
  #
  ...
  # can be authored around rather than something an execution hangs on.
  #
  ...
  # An execution this app can no longer name a chart for is a discard, with the
  ...
  # a fixture key, which is what an execution of a document that is not a shipped
  # fixture looks like.
  #
  ...
  # And an execution nobody stored is refused before anything else is read - the
