defmodule StatifierExamples.MixDepsTest do
  ...
  #
  # That default arm is the Hex requirement `{:statifier_blocks, "~> 0.7"}`.
  # Sabotage: changed the else arm of `statifier_blocks_dep/0` to
  # `{:statifier_blocks, "~> 0.6"}`; this test went red on the membership
  # assertion.
  test "with STATIFIER_BLOCKS_PATH unset the statifier_blocks dep is the Hex requirement `~> 0.7`" do
    refute System.get_env("STATIFIER_BLOCKS_PATH")

    deps = Mix.Project.config()[:deps]

    assert {:statifier_blocks, "~> 0.7"} in deps
  end
end
