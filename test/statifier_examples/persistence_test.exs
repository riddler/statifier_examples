defmodule StatifierExamples.PersistenceTest do
  ...
  open - `supports_execution_outcome?/1` and `list_execution_states_by_metadata/2` -
  ...
  # found nothing, because a match naming only `parent_execution_id` never
  # equals a stored map that also carries `invoke_id`, `child_index` and
  # `content_hash`. Reverted.
  test "list_executions_by_metadata matches both of the driver's linkage shapes", %{opts: opts} do
    insert!(
      opts,
      "run-a/call-one/0",
    ...
    assert {:ok, every_child} =
             Persistence.list_executions_by_metadata(opts, Linkage.parent_match("run-a"))

    assert Enum.map(every_child, & &1.execution_id) |> Enum.sort() ==
             ["run-a/call-one/0", "run-a/call-two/0"]

    assert {:ok, one_invocation} =
             Persistence.list_executions_by_metadata(
               opts,
               Linkage.invocation_match("run-a", "call-two")
             )

    assert Enum.map(one_invocation, & &1.execution_id) == ["run-a/call-two/0"]
  end

  # The records come back in `fetch_execution/2`'s shape, because that is what
  ...
  #
  # Sabotage: made the reduce collect the `{execution_id, metadata}` tuples it
  # filtered on instead of calling `fetch_execution/2`; this went red on the
  # status key. Reverted.
  test "list_executions_by_metadata answers records in fetch_execution's shape", %{opts: opts} do
    insert!(opts, "run-c/call/0", Linkage.to_metadata(Linkage.new("run-c", "call", 0, "h")))

    assert {:ok, [record]} =
             Persistence.list_executions_by_metadata(opts, Linkage.parent_match("run-c"))
  ...
  test "list_executions_by_metadata refuses an empty match rather than matching everything", %{
    ...
    assert_raise ArgumentError, fn -> Persistence.list_executions_by_metadata(opts, %{}) end

    assert_raise ArgumentError, fn ->
      Persistence.list_executions_by_metadata(opts, %{key: "atom"})
    end

    assert_raise ArgumentError, fn ->
      Persistence.list_executions_by_metadata(opts, "not a map")
    end
  end
  ...
  #
  # Sabotage: made `supports_execution_outcome?/1` answer `false`; this went red
  ...
  test "the three capability guards a fan-out is opened against all hold" do
    ...
    assert Storage.execution_outcome_supported?(store)
    assert Storage.execution_states_supported?(store)
  end
  ...
  test "list_execution_states_by_metadata projects execution_id, status and child_index", %{
    opts: opts
  } do
    for index <- 0..2 do
      insert!(
    ...
    assert {:ok, states} =
             Persistence.list_execution_states_by_metadata(opts, Linkage.parent_match("run-f"))

    assert Enum.sort_by(states, & &1.child_index) == [
             %{execution_id: "run-f/call/0", status: :active, child_index: 0},
             %{execution_id: "run-f/call/1", status: :active, child_index: 1},
             %{execution_id: "run-f/call/2", status: :active, child_index: 2}
           ]
  end
  ...
  test "list_execution_states_by_metadata answers nil for a run with no linkage", %{opts: opts} do
    insert!(opts, "run-g", %{"fixture" => "signup_bulk_invites"})

    assert {:ok, [state]} =
             Persistence.list_execution_states_by_metadata(opts, %{
  ...
  # assertion. Reverted.
  test "list_execution_states_by_metadata refuses an empty or badly keyed match", %{opts: opts} do
    insert!(opts, "run-h/call/0", Linkage.to_metadata(Linkage.new("run-h", "call", 0, "h")))

    assert_raise ArgumentError, fn -> Persistence.list_execution_states_by_metadata(opts, %{}) end

    assert_raise ArgumentError, fn ->
      Persistence.list_execution_states_by_metadata(opts, %{key: "atom"})
    end

    assert_raise ArgumentError, fn ->
      Persistence.list_execution_states_by_metadata(opts, "not a map")
    end
  end
end
